...
public class App {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Query query = session.createQuery("select distinct f from Flight f where f.origin.country = :origin AND f.airplane.capacity > :capacity");
            query.setParameter("origin", "USA");
        ...
        System.out.println("\n============== All airlines that use A380 airplanes =================");
        ...
        try {
            ...
            // TODO update HQL
            @SuppressWarnings("unchecked")
            Query query = session.createQuery("from Flight f where f.airplane.model = :model AND f.airline.name != :airLinesName ");
            query.setParameter("model", "747");
            query.setParameter("airLinesName", "Star Alliance");
            List<Flight> flights = query.list();
            System.out.println("Flight:  Departs:     "
                    + "                  Arrives:                        AirLIne:");
            for (Flight flight : flights) {
        ...
        try {
            ...
            Query query = session.createQuery("from Flight f where f.departureTime < '12:00:00' AND f.departureDate= '2009-08-07' ");
            List<Flight> flights = query.list();
            System.out.println("Flight:  Departs:     "
                    + "                  Arrives:");
            for (Flight flight : flights) {
